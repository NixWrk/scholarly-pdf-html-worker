...
def _declared_document_urls(html: str) -> tuple[str, ...]:
    """Collect canonical/full-html URLs declared in the document head."""

    urls: list[str] = []
    for match in _DECLARED_URL_RE.finditer(html[:500_000]):
        attrs = match.group("attrs") or ""
        href = _attr_value(attrs, "href")
        content = _attr_value(attrs, "content")
        rel = _attr_value(attrs, "rel") or ""
        name = (_attr_value(attrs, "name") or _attr_value(attrs, "property") or "").lower()
...
def _document_url_candidates(
    ...
    *,
    ...
    for raw_url in (source_url, canonical_url, *_declared_document_urls(html)):
        parsed = _urlsplit_or_none(raw_url)
