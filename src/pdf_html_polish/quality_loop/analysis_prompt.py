...
def render_llm_prompt(pack: dict[str, Any]) -> str:
    lines = [
        ...
        "",
        "You are reviewing a pdf-html-polish EN polish experiment.",
        ...
        f"- p62_source_visual_unavailable_count: `{(pack.get('p62_image_recovery_stage') or {}).get('source_visual_unavailable_count')}`",
        f"- p62_source_visual_unavailable_group_count: `{(pack.get('p62_image_recovery_stage') or {}).get('source_visual_unavailable_group_count')}`",
    ...
    if p62_recovery.get("candidate_count") or p62_recovery.get("asset_ready_count"):
        lines.extend(
            [
                ...
                f"- source_visual_unavailable_count: `{p62_recovery.get('source_visual_unavailable_count')}`",
                f"- source_visual_unavailable_group_count: `{p62_recovery.get('source_visual_unavailable_group_count')}`",
